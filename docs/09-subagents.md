# Subagents: delegar sin ensuciar la conversación principal

## Qué es un subagent

Un **subagent** es una instancia separada del agente, con su propio historial y su propia ventana de contexto, que se lanza para resolver una tarea puntual y devuelve un resultado final a la conversación principal — sin que todos los pasos intermedios que dio (búsquedas, lecturas, intentos fallidos) queden ocupando espacio en el contexto principal.

La conversación principal solo ve el **resumen/resultado final** que devuelve el subagent, no todo el proceso que usó para llegar ahí.
...
Si una tarea requiere explorar mucho (leer 20 archivos para entender cómo funciona algo, probar varias búsquedas hasta encontrar lo que se busca), delegarla a un subagent evita que esa exploración "ensucie" el contexto principal: el subagent hace el trabajo pesado en su propio espacio, y la conversación principal solo recibe la conclusión.

## Cuándo conviene delegar vs. resolver inline

**Conviene un subagent cuando:**
...
- Se pueden lanzar varias exploraciones independientes en paralelo (varios subagents a la vez, cada uno con una pregunta distinta).
...
**Conviene resolver inline (sin subagent) cuando:**
...
- Es una tarea corta: el costo de armar el contexto para un subagent (explicarle qué tiene que hacer, por qué, qué se probó ya) puede ser mayor que resolverlo directo.

Regla práctica: **no delegar entendimiento**. Un subagent no tiene el contexto de toda la conversación previa — hay que darle todo lo que necesita para trabajar de forma autónoma (qué se busca, qué se descartó ya, qué formato de respuesta se espera). Un prompt tipo "arreglá el bug" sin contexto produce resultados genéricos o directamente incorrectos.

## Agentes especializados vs. general-purpose

Los subagents pueden ser:
...
Los subagents se pueden lanzar:

- **En paralelo**: varias tareas independientes a la vez, cuando ninguna depende del resultado de la otra.
- **En segundo plano (background)**: la conversación principal sigue mientras el subagent trabaja, y se recibe una notificación cuando termina — sin quedarse esperando ni "adivinar" el resultado antes de que llegue.

La clave es no simular ni anticipar el resultado de un subagent que todavía no terminó: si se lanzó en background, su resultado llega como una notificación aparte, nunca como algo que se escribe de antemano.
